...
// Explicaciones sencillas de cada Hand Rank que otorga recompensa (mismas
// 8 manos definidas en HAND_RANKS). titleKey/textKey siguen la convención
// data-i18n existente (__ resuelto vía config.js); fallbackTitle/fallbackText
// se muestran si __ no está disponible o la clave aún no existe en el
// sistema central de idiomas.
var HAND_EXPLANATIONS = {
    royal_flush: {
        titleKey: 'escalera_real', fallbackTitle: 'Escalera Real',
        textKey: 'exp_escalera_real',
        fallbackText: 'Necesitas A, K, Q, J y 10, todas del mismo palo.'
    },
    straight_flush: {
        titleKey: 'escalera_color', fallbackTitle: 'Escalera Color',
        textKey: 'exp_escalera_color',
        fallbackText: 'Necesitas 5 cartas seguidas y del mismo palo.'
    },
    four_kind: {
        titleKey: 'poker', fallbackTitle: 'Póker',
        textKey: 'exp_poker',
        fallbackText: 'Necesitas 4 cartas con el mismo número.'
    },
    full_house: {
        titleKey: 'full', fallbackTitle: 'Full House',
        textKey: 'exp_full',
        fallbackText: 'Necesitas 3 cartas iguales y otras 2 cartas iguales entre sí.'
    },
    flush: {
        titleKey: 'color', fallbackTitle: 'Color',
        textKey: 'exp_color',
        fallbackText: 'Necesitas 5 cartas del mismo palo, sin importar el número.'
    },
    straight: {
        titleKey: 'escalera', fallbackTitle: 'Escalera',
        textKey: 'exp_escalera',
        fallbackText: 'Necesitas 5 cartas con números seguidos, sin importar el palo.'
    },
    three_kind: {
        titleKey: 'trio', fallbackTitle: 'Trío',
        textKey: 'exp_trio',
        fallbackText: 'Necesitas 3 cartas con el mismo número.'
    },
    two_pair: {
        titleKey: 'doble_par', fallbackTitle: 'Doble Par',
        textKey: 'exp_doble_par',
        fallbackText: 'Debe tener dos pares de cartas iguales.'
    }
};

function textoExplicacionHand(key) {
    var info = HAND_EXPLANATIONS[key];
    if (!info) return null;
    var titulo = (typeof __ === 'function') ? __(info.titleKey) : info.fallbackTitle;
    var texto = (typeof __ === 'function') ? __(info.textKey) : info.fallbackText;
    // Si la clave de explicación aún no existe en el sistema central de
    // idiomas, __() devuelve la propia clave: usamos el fallback en ese caso.
    if (texto === info.textKey) texto = info.fallbackText;
    return { titulo: titulo, texto: texto };
}

// Muestra/oculta la caja de explicación al pasar (hover) o tocar una fila
// de la tabla de Hand Ranks, y agranda ligeramente esa fila mientras tanto.
function initExplicacionesHandRanks() {
    var box = document.getElementById('hand-explanation-box');
    var titleEl = document.getElementById('hand-explanation-title');
    var textEl = document.getElementById('hand-explanation-text');
    if (!box || !titleEl || !textEl) return;

    var filas = document.querySelectorAll('.payout-row[data-hand-key]');
    filas.forEach(function(fila) {
        var key = fila.getAttribute('data-hand-key');

        function mostrar() {
            var contenido = textoExplicacionHand(key);
            if (!contenido) return;
            fila.classList.add('is-explaining');
            titleEl.textContent = contenido.titulo;
            textEl.textContent = contenido.texto;
            box.classList.add('active');
        }
        function ocultar() {
            fila.classList.remove('is-explaining');
            box.classList.remove('active');
        }

        fila.addEventListener('mouseenter', mostrar);
        fila.addEventListener('mouseleave', ocultar);
        fila.addEventListener('touchstart', mostrar, { passive: true });
        fila.addEventListener('touchend', ocultar);
        fila.addEventListener('touchcancel', ocultar);
    });
}
...
window.addEventListener('idiomaAplicado', function () {
    ...
    // Si la caja de explicación de Hand Ranks está abierta en este momento,
    // refresca su texto al nuevo idioma en vez de dejarlo desactualizado.
    var explicandoAhora = document.querySelector('.payout-row.is-explaining');
    var box = document.getElementById('hand-explanation-box');
    if (explicandoAhora && box && box.classList.contains('active')) {
        var contenido = textoExplicacionHand(explicandoAhora.getAttribute('data-hand-key'));
        if (contenido) {
            document.getElementById('hand-explanation-title').textContent = contenido.titulo;
            document.getElementById('hand-explanation-text').textContent = contenido.texto;
        }
    }
});