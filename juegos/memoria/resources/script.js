...
function initSlider() {
  ...
  if (!slider || !progress || !thumb || !display) return;
  let isDragging = false;
  function updateSlider(value) {
    apuestaActual = Math.max(MIN_APUESTA, Math.min(MAX_APUESTA, value));
    display.textContent = apuestaActual;
    const percent = (apuestaActual - MIN_APUESTA) / (MAX_APUESTA - MIN_APUESTA);
    const sliderWidth = slider.offsetWidth - 35;
    thumb.style.left = (percent * sliderWidth) + "px";
    progress.style.width = (percent * 100) + "%";
  }
  function getValueFromPosition(clientX) {
    const rect = slider.getBoundingClientRect();
    const percent = (clientX - rect.left - 17.5) / (rect.width - 35);
    return Math.round(MIN_APUESTA + percent * (MAX_APUESTA - MIN_APUESTA));
  }
  slider.addEventListener("mousedown", (e) => { isDragging = true; updateSlider(getValueFromPosition(e.clientX)); });
  document.addEventListener("mousemove", (e) => { if (isDragging) updateSlider(getValueFromPosition(e.clientX)); });
  document.addEventListener("mouseup", () => { isDragging = false; });
  updateSlider(apuestaActual);
}
...
function endGame() {
  ...
  if (moves <= maxMovPermitido) {
    var ganancia = apuestaActual * nivel.mult;
    if (typeof window.calcularGananciaConItems === 'function') ganancia = window.calcularGananciaConItems(ganancia, apuestaActual);
    if (typeof agregarMonedas === 'function') agregarMonedas(ganancia);
    if (winOverlay) {
